...
<head>
    ...
    <title>{{ $title ?? 'Panel Alumno - Servicio Social' }}</title>
...
<body class="h-full flex flex-col font-sans antialiased text-gray-900">
    @php
        $links = [
            ['label' => 'Mi Panel', 'route' => 'alumno.dashboard', 'active' => 'alumno.dashboard'],
            ['label' => 'Vacantes Disponibles', 'route' => 'alumno.vacantes.index', 'active' => 'alumno.vacantes.*'],
        ];
    @endphp
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex min-h-16 flex-col gap-3 py-3 sm:flex-row sm:items-center sm:justify-between sm:py-0">
                <div class="flex items-center gap-6">
                    <a href="{{ route('alumno.dashboard') }}" class="shrink-0">
                ...
                <div class="flex items-center gap-3 text-xs">
                    <span class="rounded-full bg-amber-50 px-3 py-1 font-semibold uppercase tracking-[0.2em] text-amber-700">
                        Alumno
                    </span>
